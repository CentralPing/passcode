const {config, issue, verify} = require('./token');
...
describe('[Unit] token', () => {
  describe('`config`', () => {
    ...
    it('should expose properties for setting defaults', () => {
      expect(config).toMatchObject({
        issuer: undefined,
        subject: undefined,
        salt: expect.stringMatching(v4RegExp),
        iterations: 1000,
        keyLength: 64,
        digest: 'sha512',
        encoding: 'hex',
        secret: expect.stringMatching(v4RegExp),
        codeLength: 6,
        expiresIn: '5m'
      });
    });
