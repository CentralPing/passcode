...
const unsignedTokenRegExp = /^[0-9a-zA-Z_-]+\.[0-9a-zA-Z_-]+\.$/;
...
describe('[Unit] token', () => {
  describe('`issue`', () => {
    ...
    it('should return an error with an algorithm but without a secret', () => {
      const {error} = issue({}, {algorithm: 'HS256'});
      expect(error).not.toBeNull();
      expect(error.message).toEqual(expect.stringMatching(/secret/i));
    });

    it('should issue an unsigned token without a secret', () => {
      ...
      expect(value).toMatchObject({
        ...
        token: expect.stringMatching(unsignedTokenRegExp)
      });
    });

    it('should issue a signed token with a secret', () => {
  ...
  describe('`verify`', () => {
    ...
    describe('with an usigned token', () => {
      it('should return an error without a valid token and code', () => {
        ...
        const {error: err4} = verify(token, {code: 'foo'});
    ...
    describe('with a signed token', () => {
      it('should return an error without a valid token, secret and code', () => {
        ...
        const {error: err5} = verify(token, {secret, code: 'foo'});
    ...
    describe('with custom salt', () => {
      it('should return an error with invalid value', () => {
        ...
        const {value: {token: tok4}} = issue({}, {secret}, {salt});
        const {error: err4} = verify(tok4, {secret}, {salt});
        expect(err4).not.toBeNull();
        expect(err4.message).toEqual(expect.stringMatching(/pass/i));
      });
